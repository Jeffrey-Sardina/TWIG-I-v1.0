...
class Structure_Loader():
    ...
    def build_neighbour_cache(self):
        ...
        for target in self.ents_to_triples:
            neighbour_nodes_stats[target] = {}
            neighbour_nodes_stats[target]['mean'] = np.mean(
                list(neighbour_nodes[target].values())
            )
            neighbour_nodes_stats[target]['min'] = np.min(
                list(neighbour_nodes[target].values())
            )
            neighbour_nodes_stats[target]['max'] = np.max(
                list(neighbour_nodes[target].values())
            ) 
            neighbour_nodes_stats[target]['count'] = len(neighbour_nodes[target])

        neighbour_preds_stats = {}
        for target in self.ents_to_triples:
            neighbour_preds_stats[target] = {}
            neighbour_preds_stats[target]['mean'] = np.mean(
                list(neighbour_preds[target].values())
            )
            neighbour_preds_stats[target]['min'] = np.min(
                list(neighbour_preds[target].values())
            )
            neighbour_preds_stats[target]['max'] = np.max(
                list(neighbour_preds[target].values())
            )
            neighbour_preds_stats[target]['count'] = len(neighbour_preds[target])
    ...
    def __call__(self, s, p, o):
        ...
        if self.use_2_hop_fts:
            ...
            for target_name in target_dict:
                ...
                mean_deg_neighbour = self.neighbour_nodes[target]['mean']
                ...
                mean_freq_rel = self.neighbour_preds[target]['mean']
...
def do_load(
    ...
    for dataset_name in all_datasets:
        ...
        train_X_pos, norm_func, _, n_local = calc_split_data(
            triples_dicts=triples_dicts,
            graph_stats=graph_stats,
            purpose='train',
            batch_size=batch_size,
