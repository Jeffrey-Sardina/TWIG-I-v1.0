...
def do_batch(
    ...
    # if verbose >= 2:
    #     '''
    #     In this case we obsessively print all the negative and positive triple
    #     vectors, as was as the positive / negative flag
    #     '''
    #     print('<<' + '=' * 50)
    #     for i in range(len(X_pos)):
    #         pos_vec = X_pos[i]
    #         pos_items = ','.join(str(round(float(x), 3)) for x in pos_vec)
    #         print('<' + '-' * 30 + f'{i} / {len(X_pos)}')
    #         print(f"POSITIVE,{pos_items}")
    #         for j in range(npps[i]):
    #             neg_vec = X_neg[j]
    #             neg_items = ','.join(str(round(float(x), 3)) for x in neg_vec)
    #             print(f"NEGATIVE,{neg_items}")
    #         print(str(i) + '-' * 30 + '>')
    #     print('=' * 50 + '>>')
...
def run_training(
    ...
    '''
    model.to(device)
    loss_fn = get_margin_ranking_loss(reduction='mean', margin=0.1)
