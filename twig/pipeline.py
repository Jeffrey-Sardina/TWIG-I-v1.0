...
import torch
from utils import load_custom_dataset, Custom_Dataset
    ...
    for run_id, hps in grid:
        ...
        try:
            ...
            if len(glob.glob(f'{exp_dir}/*')) > 0: # if its data was written already
                send(f'Skipping exp {run_id} -- already completed')
                continue

        send(f'Starting exp with run_id {run_id}')
        pipeline_result, evaluator = run_exp(hps, exp_dir, dataset, model, seed)
        send(f'Finished exp with run_id {run_id}')
...
def main(out_file, out_dir, num_processes, dataset, model, seed, sub_grid_range=None):
    grid = get_hp_grid()
...
if __name__ == '__main__':
    ...
    main(out_file, out_dir, num_processes, dataset, model, seed, sub_grid_range=sub_grid_range)
