...
def do_batch(
    ...
    if batch % 500 == 0 and verbose:
        ...
        send(f"batch {batch} / {num_total_batches} loss: {loss_val.item()}")
    ...
    **CRITICAL NOTE**: This function puts the model into eval() mode, but does
    ...
    with torch.no_grad():
        ...
        for dataloader_name in dataloaders:
            ...
            for i in range(len(dataloader_pos)):
...
def run_training(
    ...
    The arguments it accepts are:
        ...
        - verbose (bool): whether to output loss values occaisonally at certain
          batch numbers.
        - model_name_prefix (str): not used.
        - checkpoint_dir (str): not used.
    ...
    for t in range(epochs):
        send(f"REC: [1] Epoch {t+1} -- ")
        print(f"Epoch {t+1} -- ", end='')
        train_epoch(training_dataloaders,
                model,
                loss_fn,
                npp,
                negative_samplers,
        ...
        if (t+1) % checkpoint_every_n == 0:
            print(f'NOT Saving checkpoint at epoch {t+1}')
            # state_data = f'e{t+1}'
            # torch.save(
            #     model,
            #         os.path.join(
