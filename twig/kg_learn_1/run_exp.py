...
from negative_sampler import Simple_Negative_Sampler
import pykeen
...
def load_dataset(
    ...
    send('REC: loading dataset')

    dataloaders, norm_basis, raw_X_train_pos = do_load(
        dataset_names,
...
def load_filters(
    ...
    for dataset_name in dataset_names:
        ...
        filters[dataset_name] = {
            'train': triples_dicts['train'] if use_train_filter else set(), #train triples
            'valid': triples_dicts['train'] + triples_dicts['valid'] if use_valid_and_test_filters else set(), #all train and valid triples
            'test': triples_dicts['all'] if use_valid_and_test_filters else set() #all train, valid, and test triples
        }
...
def load_negative_samplers(
    ...
    send('REC: loading negative samplers')
    ...
    for dataset_name in dataset_names:
        ...
        simple_sampler = Simple_Negative_Sampler(
            filters[dataset_name],
            graph_stats,
...
def train_and_eval(
    ...
    send('REC: running training and eval')
    run_training(model,
        training_dataloaders,
        testing_dataloaders,
        valid_dataloaders,
...
def main(
    ...
    send(f'REC: starting with v{version} and datasets {dataset_names}')

    checkpoint_dir = 'checkpoints/'
    model_name_prefix = f'v{version}_{"-".join(d for d in dataset_names)}'
    model = load_nn(version)

    dataloaders, norm_basis, raw_X_train_pos = load_dataset(
        dataset_names,
        normalisation=normalisation,
        batch_size=batch_size,
    )
    ...
    negative_samplers = load_negative_samplers(
        ...
        raw_X_train_pos=raw_X_train_pos
    )
...
if __name__ == '__main__':
    ...
    npp = int(sys.argv[7])
    use_train_filter = sys.argv[8] == '1'
    use_valid_and_test_filters = sys.argv[9] == '1'
    sampler_type = sys.argv[10]
    if len(sys.argv) > 11:
        hyp_validation_mode = sys.argv[11] == '1'
    else:
        hyp_validation_mode = False
