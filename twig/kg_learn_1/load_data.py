...
def get_twm_data_augment(
    ...
    for perc in percentiles_wanted:
        global_struct[f'node_deg_p_{perc}'] = metadata['percentiles'][perc]
        global_struct[f'rel_freq_p_{perc}'] = metadata['total_rel_degree_percentiles'][perc]
    global_struct_vec = list(global_struct.values())  
    ...
    for triple_idx in triples_map:
        ...
        all_data_pos.append([triple_idx] + global_struct_vec + data_pos)
    X_p = torch.tensor(all_data_pos, dtype=torch.float32)

    return X_p

def do_norm(to_norm, basis, normalisation='none'):
    '''
    do_norm() does the requested normalisation on the input data, using a
    separate tensor as the basis for normalisation.

    The arguments it accepts are:
        - to_norm (torch.Tensor): the data to be normalised.
        - basis (torch.Tensor): the Tensor whose values for a basis for
    ...
    The values it returns are:
        - norm_data (torch.Tensor) the normalised data, or the original data is
          normalisation is "none".
    '''
    if normalisation == 'none':
        norm_data = to_norm
    elif normalisation == 'minmax':
        norm_data = minmax_norm(to_norm, basis)
    elif normalisation == 'zscore':
        norm_data = zscore_norm(to_norm, basis)
    else:
        assert False, f'invalid normalisation method: {normalisation}'
    
    return norm_data

def minmax_norm(base_data, all_concat):
    '''
    minmax_norm() performs minmax normalisation of the input data using
    all_concat as the basis for calculating mean and st dev. All normalisation
    is done on rows as atomic units of data, representing input feature
    vectors.
    
    The arguments it accepts are:
        - base_data (torch.Tensor): the data to be normalised.
        - all_concat (torch.Tensor): the Tensor whose values for a basis for
          normalisation. It's values are the used to compute the normalisation
          parameters (such as min and max values for minmax normalisation). As an
          example, when normalising the test set, you want to normalise it
          relative to the values **in the train set** so as to avoid data
    ...
    The values it returns are:
        - norm_data (torch.Tensor) the normalised data.
    '''
    train_min = torch.min(all_concat, dim=0).values[1:]
    train_max = torch.max(all_concat, dim=0).values[1:]
    norm_data = {}
    for dataset_name in base_data:
        norm_data[dataset_name] = {}
        dataset_data = base_data[dataset_name]
        for run_id in dataset_data:
            norm_data[dataset_name][run_id] = {}
            X = dataset_data[run_id]['X']
            X_graph, X_other = X[:, :1], X[:, 1:] # ignore col 0; that is thetriple idx, and we needs its original value!
            X_other = (X_other - train_min) / (train_max - train_min)
            X_norm = torch.concat(
                [X_graph, X_other],
                dim=1
            )
            ...
            norm_data[dataset_name][run_id]['X'] = X_norm
            norm_data[dataset_name][run_id]['y'] = dataset_data[run_id]['y']
    return norm_data

def zscore_norm(base_data, all_concat):
    '''
    zscore_norm() performs z-score normalisation of the input data using
    all_concat as the basis for calculating mean and st dev. All normalisation
    is done on rows as atomic units of data, representing input feature
    vectors.
    ...
    The arguments it accepts are:
        ...
        - all_concat (torch.Tensor): the Tensor whose values for a basis for
          normalisation. It's values are the used to compute the normalisation
          parameters (such as mean and std values for zscore normalisation). As an
          example, when normalising the test set, you want to normalise it
          relative to the values **in the train set** so as to avoid data
          leakage. In this case the norm_basis would be be training tensor.
    ...
    The values it returns are:
        - norm_data (torch.Tensor) the normalised data.
    '''
    train_mean = torch.mean(all_concat, dim=0)[1:]
    train_std = torch.std(all_concat, dim=0)[1:]
    norm_data = {}
    for dataset_name in base_data:
        norm_data[dataset_name] = {}
        dataset_data = base_data[dataset_name]
        for run_id in dataset_data:
            norm_data[dataset_name][run_id] = {}
            X = dataset_data[run_id]['X']
            X_graph, X_other = X[:, :1], X[:, 1:] # ignore col 0; that is the triple idx, and we needs its original value!
            X_other = (X_other - train_mean) / train_std
            X_norm = torch.concat(
                [X_graph, X_other],
                dim=1
            )
...
def calc_split_data(
    ...
    if norm_basis is None:
        # TODO -- reduce the size needed for the norm basis -- we should not have to use the whole tensor
        # see rec_construct_2 for how
        norm_basis = X_p #if this is X_p concat'd to X_n, results may be different in theory and practice
    X_p = do_norm(to_norm=X_p, basis=norm_basis, normalisation=normalisation)
    ...
    return dataloader_pos, norm_basis, X_p
...
def do_load(
    ...
    for dataset_name in all_datasets:
        ...
        train_X_pos, norm_basis, raw_X_train_pos = calc_split_data(
            triples_dicts,
            graph_stats,
            purpose='train',
            norm_basis=None,
    ...
    norm_basis = norm_basis.to(device)
    return dataloaders, norm_basis, raw_X_train_pos
